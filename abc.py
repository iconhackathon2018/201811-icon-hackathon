...
class ABCToken(IconScoreBase):
    ...
    __SCORE_VERSION = 'v0.0.4'

    # ==================================================================================================================
    # PROPERTY for ArrayDB
    # ==================================================================================================================
    ...
    def fallback(self):
        ...
        # EVENT
        self.event_transfer(account, amount)
    ...
    def bet(self, amount: int, roll_under: int, house_hash: str, player_seed: str):
        ...
        # PRECONDITION
        self._precondition(0 < available_amount, f'Not enough available amount to bet --- cause by : {available_amount}')
        self._precondition(0 < amount, f'Bet amount must be positive numbers only --- cause by : {amount}')
        self._precondition(amount <= available_amount, f"Bet amount can not be over player's available amount --- cause by: {available_amount}")
        self._precondition(self._context__bet_max.get() >= amount >= self._context__bet_min.get(), 'Invalid amount')
        self._precondition(self._context__roll_under_max.get() >= roll_under >= self._context__roll_under_min.get(), 'Invalid roll under')
        ...
        # EVENT
        self.event_bet(str(index))
        self.event_amount(amount)
        self.event_dividend(dividend)
    ...
    def get_bet(self, index: int) -> dict:
        ...
        """
        return {
            'acount': self._bet__account.get(index),
    ...
    def roll(self, index: int, house_seed: str):
        ...
        if dice_number < roll_under:
            ...
            self.event_roll_success(account, dividend, dice_number)

        self._bet__is_active[index] = BET_DONE

        # EVENT
        self.event_roll(str(result))  # (0: FAILED, 1: SUCCESS)
    ...
    @eventlog(indexed=2)
    def event_transfer(self, account: Address, amount: int):
        pass

    @eventlog(indexed=1)
    def event_bet(self, index: str):
        pass

    @eventlog(indexed=1)
    def event_amount(self, amount: int):
        pass

    @eventlog(indexed=1)
    def event_dividend(self, dividend: int):
        pass

    @eventlog(indexed=1)
    def event_roll(self, result: str):
        pass

    @eventlog(indexed=3)
    def event_roll_success(self, account: Address, dividend: int, dice_number: int):
        pass
